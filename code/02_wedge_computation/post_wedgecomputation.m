...
% No pi_f
pi_f = ones(999,1) .* pi_f1;
save(fullfile(data_out,'Nopifpaths.mat'), ...
     'A','pi_h','pi_f','pi_x','pi_g','pi_n','k','y','x','l','c','sl');
pi_f = pick_endo('pi_f',1:999);
% No pi_h
pi_h = ones(999,1) .* pi_h1;
save(fullfile(data_out,'Nopihpaths.mat'), ...
     'A','pi_f','pi_h','pi_x','pi_g','pi_n','k','y','x','l','c','sl');
pi_h = pick_endo('pi_h',1:999);
% No pi_x
pi_x = ones(999,1) .* pi_x1;
save(fullfile(data_out,'Nopixpaths.mat'), ...
     'A','pi_f','pi_h','pi_x','pi_g','pi_n','k','y','x','l','c','sl');
pi_x = pick_endo('pi_x',1:999);
% No pi_g
pi_g = ones(999,1) .* pi_g1;
save(fullfile(data_out,'Nopigpaths.mat'), ...
     'A','pi_f','pi_h','pi_x','pi_g','pi_n','k','y','x','l','c','sl');
pi_g = pick_endo('pi_g',1:999);
% No pi_n
pi_n = ones(999,1) .* pi_n1;
...
plot(timeline, log(A_t(1:T)./A_bar), 'Color',[0 0 1], 'LineWidth', 3);
...
% Firm labor wedge
f = figure('Visible','off');
plot(timeline, log(pi_f_t(1:T)./pi_f_bar), 'Color',[0 0 1], 'LineWidth', 3);
set(gca,'XTick',1967:3:2020);
set(gca,'YLim',[-0.6 1]);
ylabel('logs','FontSize',12,'FontWeight','bold');
xlabel('Year','FontSize',12,'FontWeight','bold');
...
% Investment wedge
f = figure('Visible','off');
plot(timeline, log(pi_x_bar./pi_x_t(1:T)), 'Color',[0 0 1], 'LineWidth', 3);
set(gca,'XTick',1967:3:2020);
ylabel('logs','FontSize',12,'FontWeight','bold');
xlabel('Year','FontSize',12,'FontWeight','bold');
grid on
...
f = figure('Visible','off');
plot(timeline, log(pi_h_t(1:T)./pi_h_bar), 'Color',[0 0 1], 'LineWidth', 3);
set(gca,'XTick',1967:3:2020);
set(gca,'YLim',[-0.6 1]);
ylabel('logs','FontSize',12,'FontWeight','bold');
xlabel('Year','FontSize',12,'FontWeight','bold');
...
% Resource wedge
f = figure('Visible','off');
plot(timeline, log(pi_g_t(1:T)./pi_g_bar), 'Color',[0 0 1], 'LineWidth', 3);
set(gca,'XTick',1967:3:2020);
ylabel('logs','FontSize',12,'FontWeight','bold');
xlabel('Year','FontSize',12,'FontWeight','bold');
grid on
